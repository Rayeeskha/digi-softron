...
class TeamRepository extends BaseRepository
{
    ...
    public function getData(){
        $data = Team::select('id','name','designation','image','profile_url','status','created_at',\DB::raw('@rownum  := @rownum  + 1 AS DT_RowIndex'))->orderBy('id', 'DESC');
        return Datatables::of($data)->addIndexColumn()
                ->addColumn('action', function($row){
                    $btn = '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" class="edit btn btn-info btn-md editTeam"><span class=" bx bx-edit text-white"></span></a>'; 
    ...
    public function store($data){
        $id = $data['id'];
        if (!empty($data['image'])) {
            $data['image'] =  $this->uploadImage($data['image'], 'uploads/images/teams', 'teams', ['id' => $id ], 'image');
        }
        Team::updateOrCreate(['id' => $id], $data);
